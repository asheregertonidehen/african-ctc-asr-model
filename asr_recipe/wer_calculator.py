...
ModalityInput = None  # unused in CTC path; omnilingual_asr 0.1.0 (PyPI) ships no
# wav2vec2_llama.syntax module -- this recipe is from git-main which is ahead of
# the release. Only used as a `List[ModalityInput]` type hint below (LLM path),
# never instantiated, so aliasing to None is safe for CTC finetuning.
