...
class AudioTableProcessor:
    ...
    def _post_process(self, data: Dict[str, Any] | None) -> Dict[str, Any] | None:
        ...
        # Some clips "decode" but report sample_rate<=0 or an empty waveform --
        # resample() then throws ("frequency should be positive") and, since this
        # runs inside a Ray task, aborts the WHOLE ingest. Drop them instead.
        if sr is None or sr <= 0 or wav is None or wav.numel() == 0:
            return None
    ...
    def _wav_to_bytes(
        ...
        """
        # Encode with soundfile directly, as FLAC. We deliberately bypass
        # torchaudio.save(): on this pod's torchaudio (2.9, torchcodec dispatch)
        # `torchaudio.save(BytesIO, format="ogg", backend="soundfile")` SEGFAULTS
        # the whole Ray worker -- a native crash no try/except can catch. FLAC via
        # libsndfile is stable and thread-safe (independent buffers), and fairseq2's
        # AudioDecoder reads FLAC bytes downstream just like it read ogg.
        if wav is None:
            return None
        if isinstance(wav, torch.Tensor):
            wav = wav.detach().cpu().numpy()
        wav = np.asarray(wav, dtype=np.float32).reshape(-1)  # mono, 1-D
        # Guard the inputs that can themselves crash the encoder.
        if wav.size == 0 or not np.isfinite(wav).all():
            return None
        buffer = io.BytesIO()
        sf.write(buffer, wav, self.sample_rate, format="FLAC")
    ...
    def read_audio_bytes(
        ...
        # num_parallel_calls: this was the ingest bottleneck -- the file-path
        # variant above always decoded on nb_threads threads, but this bytes
        # variant (the one the HF ingestion actually uses) ran single-threaded.
        builder = builder.map(
            self._bytes_decoded_audio, num_parallel_calls=self.nb_threads
        )
        builder = builder.map(self._post_process, num_parallel_calls=self.nb_threads)
    ...
    def __call__(self, table: pa.Table) -> pa.Table:
        ...
        # Encode on a thread pool -- the ogg encode releases the GIL and this
        # serial comprehension was (with the serial decode above) why ingest
        # crawled at ~3 clips/s on one CPU.
        with ThreadPoolExecutor(max_workers=self.nb_threads) as pool:
            audio_bytes = list(pool.map(self._wav_to_bytes, audio_waveforms))
        ...
        # Drop rows whose audio failed to decode (audio_size is null). A null
        # audio_size reads back as NaN (float) and crashes fairseq2's length/
        # bucketing logic during training/validation ("'length' ... is of type
        # float"). Better to never write these rows than to poison the dataset.
        return table.filter(pc.is_valid(table["audio_size"]))
